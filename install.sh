...
help() {
  cat <<'EOF'
Install a binary release of bitomc hosted on GitHub
...
crate=bitomc
url=https://github.com/BitOMC/BitOMC
...
if [ -z ${tag-} ]; then
  ...
  tag=$(curl --proto =https --tlsv1.2 -sSf https://api.github.com/repos/BitOMC/BitOMC/releases/latest |
    grep tag_name |
    cut -d'"' -f4
  )
fi
...
if [ -z ${target-} ]; then
  ...
  case $uname_target in
    ...
    *)
      ...
      say 'Consider opening an issue or building from source: https://github.com/BitOMC/BitOMC'
      exit 1
    ;;
  esac
fi
